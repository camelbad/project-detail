...
void addStudent(long id, BST * recorder)//add student
{
	recorder->root = insert_bst_node(recorder->root, id);
}


void deleteStu(long id, BST *recorder)//delete student
{
	recorder->root = delete_bst_node(recorder->root, id);

}

void addCourseToStu(char *courseName, long id, BST *recorder)//enroll course
{
	
	BSTNodePtr a=searchById(recorder,id);//locate student by id
...
void delCourse(char *courseName, long id, BST *recorder)//delete course
{
	BSTNodePtr a = searchById(recorder, id);//locate student
...
void displayStuListOfCourse(char *courseName, BSTNodePtr recorderRoot)//ger the student number of certain course
{
...
void displayCoursesOfStu(long id, BST *recorder)//Shows all courses registered for a particular student
{
	BSTNodePtr result= find_bst_node(recorder->root, id);//Target students, based on ID
...
void main()
{
	...
	BST *recorder= creatNewBst();//initializing BST tree for recorder
